...
RESULT_LOG="/tmp/webhook_result.log" # File mới để bạn theo dõi trạng thái gửi
...
# 2. Lấy IP của VPS (Thêm timeout 5s để tránh treo script nếu mạng lỗi)
...
while true; do
    ...
    if [ -s "$TEMP_LOG" ]; then
        ...
        # Gửi cURL, ẩn output nhưng bắt lại mã trạng thái HTTP
        RESPONSE=$(curl -s -w "\nHTTP_STATUS: %{http_code}" -X POST "$PHP_URL" \
             -H "Content-Type: application/json" \
             -d "{\"vps_ip\":\"$VPS_IP\", \"batch\": true, \"log\":\"$LOG_CONTENT\"}")
             
        # Ghi kết quả vào file nhật ký để bạn có thể kiểm tra
